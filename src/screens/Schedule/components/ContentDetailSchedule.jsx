import { Avatar, Button, InputBase, InputLabel, Select } from '@material-ui/core';
...
import { createExam, listExam } from '../../../actions/examActions';
...
const ContentDetailSchedule = (props) => {
  ...
  const addScheduleHandler = (e) => {
    ...
    dispatch(createExam({ ...schedule, attendants: data, user: userInfo._id }));
  ...
  return (
    <form onSubmit={addScheduleHandler} className={classes.root}>
        ...
        <FormControl component="fieldset">
          <FormControlLabel
            control={
              <Switch
                checked={true}
                onChange={(e) => setSchedule({ ...exam, status: e.target.checked })}
                name="status"
                color="primary"
              />
            }
      ...
      <form className={classes.time}>
        ...
        />
        <TextField disabled value={55} style={{ width: 70 }} id="outlined-basic" label="Time" variant="outlined" />
      </form>
